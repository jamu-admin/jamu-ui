...
/// Response from /model/info endpoint (Jamu/LiteLLM specific)
...
pub struct State {
    ...
    /// Models fetched dynamically from /model/info endpoint (Jamu mode only)
...
impl State {
    ...
    /// Fetch available models from /model/info endpoint (Jamu/LiteLLM specific)
    fn fetch_models(&mut self, cx: &mut Context<Self>) -> Task<Result<()>> {
        ...
        cx.spawn(async move |this, cx| {
            ...
            // Build /model/info URL
            let model_info_url = if api_url.ends_with("/v1") {
                api_url.replace("/v1", "/model/info")
            } else if api_url.ends_with("/v1/") {
                api_url.replace("/v1/", "/model/info")
            } else {
                format!("{}/model/info", api_url.trim_end_matches('/'))
            };
            ...
            if !response.status().is_success() {
                log::warn!("Failed to fetch models from {}: {}", model_info_url, response.status());
                return Ok(());
            }
            ...
            let model_info: ModelInfoResponse = serde_json::from_str(&body)?;

            let models: Vec<AvailableModel> = model_info
                .data
                .into_iter()
                .map(|entry| AvailableModel {
                    name: entry.model_name,
                    display_name: entry.model_info.display_name,
                    max_tokens: entry.model_info.max_input_tokens.unwrap_or(200_000),
                    max_output_tokens: entry.model_info.max_output_tokens,
                    max_completion_tokens: entry.model_info.max_output_tokens,
                    capabilities: ModelCapabilities {
                        tools: entry.model_info.supports_function_calling,
                        images: entry.model_info.supports_vision,
            ...
            log::info!("🎵 Jamu: Fetched {} models from API", models.len());
            for model in &models {
                log::info!("  - {} ({})", model.display_name.as_ref().unwrap_or(&model.name), model.name);
            }

            this.update(cx, |state, cx| {
                state.fetched_models = Some(models);
                cx.notify();
            })?;
    ...
    /// Get available models - prefer fetched, fallback to settings
    fn available_models(&self) -> &[AvailableModel] {
        if let Some(ref fetched) = self.fetched_models {
            fetched.as_slice()
        } else {
        ...
        let state = cx.new(|cx| {
            cx.observe_global::<SettingsStore>(|this: &mut State, cx| {
                ...
                if &this.settings != &settings {
                    ...
                    // Clear fetched models when settings change
                    this.fetched_models = None;
                    cx.notify();
                }
            })
            .detach();
...
impl LanguageModelProvider for OpenAiCompatibleLanguageModelProvider {
    ...
    fn authenticate(&self, cx: &mut App) -> Task<Result<(), AuthenticateError>> {
        ...
        let is_jamu_mode = std::env::var("JAMU_AGENT_MODE").unwrap_or_default() == "true";

        cx.spawn(async move |cx| {
            // First authenticate (load API key)
            let auth_result = state
                .update(cx, |state, cx| state.authenticate(cx))?
                .await;

            // If in Jamu mode and authenticated, fetch models from API
            if is_jamu_mode && auth_result.is_ok() {
                if let Some(fetch_task) = state
                    .update(cx, |state, cx| {
                        if state.is_authenticated() && state.fetched_models.is_none() {
...
impl ConfigurationView {
    ...
    fn save_api_key(&mut self, _: &menu::Confirm, window: &mut Window, cx: &mut Context<Self>) {
        ...
        cx.spawn_in(window, async move |_, cx| {
            ...
            // In Jamu mode, fetch models after setting API key
            if is_jamu_mode {
                if let Some(fetch_task) = state
                    .update(cx, |state, cx| Some(state.fetch_models(cx)))
                    .ok()
                    .flatten()
                {
                    let _ = fetch_task.await;
    ...
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let api_key_section = if self.should_render_editor(cx) {
            v_flex()
                ...
                .child(Label::new("To use Jamu with this provider, you need to add an API key."))
                .child(
                    div()
                ...
                .child(
                    Label::new(
                        format!("You can also assign the {env_var_name} environment variable and restart Jamu."),
                    )
