                    ...
                    PopoverMenu::new("codestral")
                        .menu(move |window, cx| {
                            if has_api_key {
                                ...
                                Some(ContextMenu::build(window, cx, |menu, _, _| {
                                    let fs = fs.clone();
                                    menu.entry("Use Jamu AI instead", None, move |_, cx| {
                                        set_completion_provider(
                                            fs.clone(),
                                            cx,
                                            EditPredictionProvider::Zed,
...
impl EditPredictionButton {
    ...
    fn build_copilot_context_menu(
        ...
        ContextMenu::build(window, cx, |menu, window, cx| {
            self.build_language_settings_menu(menu, window, cx)
                .separator()
                .entry("Use Jamu AI instead", None, {
                    let fs = self.fs.clone();
                    move |_window, cx| {
                        set_completion_provider(fs.clone(), cx, EditPredictionProvider::Zed)
                    }
    ...
    fn build_codestral_context_menu(
        ...
        ContextMenu::build(window, cx, |menu, window, cx| {
            self.build_language_settings_menu(menu, window, cx)
                .separator()
                .entry("Use Jamu AI instead", None, move |_, cx| {
                    set_completion_provider(fs.clone(), cx, EditPredictionProvider::Zed)
                })
                .separator()
        ...
        ContextMenu::build(window, cx, |mut menu, window, cx| {
                ...
                .and_then(|provider| provider.usage(cx))
            {
                ...
                menu = menu
                    ...
                    .entry("Upgrade to Jamu Pro or contact us.", None, |_window, cx| {
                        cx.open_url(&zed_urls::account_url(cx))
                    })
                    .separator();
