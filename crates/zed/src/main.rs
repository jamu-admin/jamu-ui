...
pub fn main() {
    ...
    app.run(move |cx| {
            ...
            .and_then(|request| OpenRequest::parse(request, cx).log_err())
        {
            ...
            None => {
                // Jamu: Check for agent mode - create minimal agent-only workspace
                let agent_mode = std::env::var("JAMU_AGENT_MODE")
                    .unwrap_or_else(|_| "false".to_string()) == "true";
                
                if agent_mode {
                    // Agent mode: Create minimal workspace with only agent panel
                    cx.spawn({
                        let app_state = app_state.clone();
                        async move |cx| {
                            if let Err(e) = create_minimal_agent_workspace(app_state, cx).await {
                                fail_to_open_window_async(e, cx)
                            }
                        }
                    })
                    ...
                    // Standard Zed mode - proceed normally
                    cx.spawn({
                        let app_state = app_state.clone();
                        async move |cx| {
                            if let Err(e) = restore_or_create_workspace(app_state, cx).await {
...
// Jamu: Create a minimal workspace with only the agent panel visible
async fn create_minimal_agent_workspace(app_state: Arc<AppState>, cx: &mut AsyncApp) -> Result<()> {
    cx.update(|cx| {
        workspace::open_new(
            Default::default(),
            app_state.clone(),
            cx,
            |workspace, window, cx| {
                // Open and focus ONLY the Agent Panel
                // This will open the left dock and activate the agent panel
                workspace.open_panel::<AgentPanel>(window, cx);
                workspace.focus_panel::<AgentPanel>(window, cx);
                
                // Close OTHER docks (right and bottom), but keep left dock open for agent
                ...
                // The workspace render will hide the center pane in agent mode
                // No need to create any editors
            },
        )
    })?
    .await?;
