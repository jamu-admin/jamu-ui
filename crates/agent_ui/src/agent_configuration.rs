                ...
                h_flex()
                    ...
                    .child(
                        v_flex()
                            ...
                            .child(
                                Label::new("Add at least one provider to use AI-powered features with Jamu.")
                                    .color(Color::Muted),
                            ),
                    ),
            )
                                    ...
                                    move |state, _window, cx| {
                                        ...
                                        update_settings_file(fs.clone(), cx, {
                                            ...
                                            move |settings, _| {
                                                ...
                                                // as that would overwrite Custom server configs with
                                                // empty Extension configs
                                                if let Some(server_settings) = settings
                                                    .project
                                                    .context_servers
                                                    .get_mut(&context_server_id.0)
                                                {
                                                    server_settings.set_enabled(is_enabled);
                                                }
                                            }
                                        });
                                    }
                ...
                div()
                    ...
                    .child(
                        v_flex()
                            ...
                            .when(
                                std::env::var("JAMU_AGENT_MODE").unwrap_or_default() != "true",
                                |this| this.child(self.render_general_settings_section(cx))
                            )
                            .when(
                                std::env::var("JAMU_AGENT_MODE").unwrap_or_default() != "true",
                                |this| this.child(self.render_agent_servers_section(cx))
                            )
