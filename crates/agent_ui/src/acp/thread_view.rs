...
impl AcpThreadView {
    pub fn new(
        ...
        let placeholder = if agent.name() == "Jamu Agent" {
    ...
    fn render_unsupported(
        ...
        let (heading_label, description_label) = (
            format!("Upgrade {} to work with Jamu", self.agent.name()),
            if version.is_empty() {
                format!(
        ...
        v_flex()
            ...
            .child(
                h_flex()
                    ...
                    .flex_wrap()
                    .justify_between()
                    .when(std::env::var("JAMU_AGENT_MODE").unwrap_or_default() != "true", |this| {
                        // Hide follow toggle and burn mode in agent mode
                        this.child(
                            h_flex()
                    ...
                    .child(
                        h_flex()
                            ...
                            .when(std::env::var("JAMU_AGENT_MODE").unwrap_or_default() != "true", |this| {
                                // Hide profile and mode selectors in agent mode
    ...
    fn render_token_usage(&self, cx: &mut Context<Self>) -> Option<Div> {
        ...
        // Try to get actual usage, otherwise show just max tokens from model
        let (used, max) = if let Some(usage) = acp_thread.token_usage() {
            (
                crate::text_thread_editor::humanize_token_count(usage.used_tokens),
            ...
            (
                "?".to_string(),
                crate::text_thread_editor::humanize_token_count(max_tokens),
            )
        };
    ...
    fn get_current_model_name(&self, cx: &App) -> SharedString {
        // For native agent (Jamu Agent), use the specific model name (e.g., "Claude 3.5 Sonnet")
    ...
    fn render_payment_required_error(&self, cx: &mut Context<Self>) -> Callout {
        const ERROR_MESSAGE: &str =
            "You reached your free usage limit. Upgrade to Jamu Pro for more prompts.";
    ...
    fn render_model_request_limit_reached_error(
        ...
        let error_message = match plan {
            ...
            | cloud_llm_client::Plan::V1(PlanV1::ZedFree) => "Upgrade to Jamu Pro for more prompts.",
